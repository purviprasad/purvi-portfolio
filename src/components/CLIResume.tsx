...
export default function CLIResume({ open = false, onClose }: Props) {
      ...
      switch (base.toLowerCase()) {
        ...
        case "projects":
          ...
          for (const p of resumeData.projects) {
            await typeOut(
              `${p.id ?? "(no-id)"}: ${p.title} — ${p.short ?? p.description ?? ""
              }\n`,
              10
            );
          }
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 transition-opacity ${fullscreen ? "bg-black/60" : "pointer-events-none"
          } z-60`}
        aria-hidden
      />
